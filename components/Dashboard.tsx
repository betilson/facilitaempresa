...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    const infoProductsRef = useRef<HTMLDivElement>(null);
    ...
    // Auto-scroll for promoted products - scrolling RIGHT
    useEffect(() => {
        const container = promotedProductsRef.current;
        if (!container || promotedProducts.length === 0) return;
    ...
    // Auto-scroll for info products - scrolling LEFT
    useEffect(() => {
        const container = infoProductsRef.current;
        if (!container || products.length === 0) return;

        // Initial scroll to end to allow leftward scrolling
        // We use a small timeout to ensure the DOM is fully rendered
        ...
        const interval = setInterval(() => {
            const currentScroll = container.scrollLeft;
            const slideWidth = 236; // 220px + 16px gap
        ...
        <div className="h-full bg-gray-50 dark:bg-gray-900 overflow-y-auto pb-20 no-scrollbar relative transition-colors duration-300">
            ...
            <div className="max-w-7xl mx-auto w-full">
                ...
                {promotedProducts.length > 0 && (
                    <div className="mb-8">
                        <div ref={promotedProductsRef} className="flex overflow-x-auto gap-4 px-6 pb-4 snap-x snap-mandatory no-scrollbar">
                ...
                {products.length > 0 && (
                    <div className="mb-8">
                        ...
                        <div ref={infoProductsRef} className="flex gap-4 overflow-x-auto px-6 no-scrollbar pb-2 snap-x snap-mandatory">
