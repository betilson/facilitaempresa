...
const App: React.FC = () => {
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'MARKET':
                return <Marketplace user={user} products={products} onSelectProduct={setSelectedProduct} onOpenPublish={() => { setBranchManageId(null); setBranchManageName(null); openPublishModal(); }} onViewPlans={() => { setProfileInitialView('UPGRADE'); setActiveTab('PROFILE'); }} />;
            case 'PROFILE':
    ...
    return (
        <div className={`h-screen w-full flex bg-gray-50 dark:bg-gray-900 transition-colors duration-300 overflow-hidden`}>
            ...
            <div className="flex-1 flex flex-col h-full relative w-full max-w-[100vw]">
                ...
                {/* Bottom Menu - Enhanced for Mobile Visibility */}
                <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t-2 border-gray-200 dark:border-gray-700 flex items-center justify-around px-4 pt-3 pb-safe z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] dark:shadow-[0_-8px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl bg-white/95 dark:bg-gray-900/95 transition-colors duration-300" style={{ paddingBottom: 'max(1.75rem, env(safe-area-inset-bottom))' }}>
                    <button onClick={() => navigateTo('HOME')} className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300 active:scale-95 ${activeTab === 'HOME' ? 'text-indigo-600 -translate-y-2 dark:text-indigo-400' : 'text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}><div className={`p-2 rounded-xl transition-all ${activeTab === 'HOME' ? 'bg-indigo-100 dark:bg-indigo-900/60 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40' : 'bg-transparent'}`}><Home size={24} className={activeTab === 'HOME' ? 'fill-indigo-600 dark:fill-indigo-400' : ''} /></div>{activeTab === 'HOME' && <span className="text-[10px] font-bold mt-1 animate-fade-in">Início</span>}</button>
                    <button onClick={() => navigateTo('MAP')} className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300 active:scale-95 ${activeTab === 'MAP' ? 'text-indigo-600 -translate-y-2 dark:text-indigo-400' : 'text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}><div className={`p-2 rounded-xl transition-all ${activeTab === 'MAP' ? 'bg-indigo-100 dark:bg-indigo-900/60 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40' : 'bg-transparent'}`}><Map size={24} className={activeTab === 'MAP' ? 'fill-indigo-600 dark:fill-indigo-400' : ''} /></div>{activeTab === 'MAP' && <span className="text-[10px] font-bold mt-1 animate-fade-in">Mapa</span>}</button>
                    <button onClick={() => navigateTo('MARKET')} className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300 active:scale-95 ${activeTab === 'MARKET' ? 'text-indigo-600 -translate-y-2 dark:text-indigo-400' : 'text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}><div className={`p-2 rounded-xl transition-all ${activeTab === 'MARKET' ? 'bg-indigo-100 dark:bg-indigo-900/60 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40' : 'bg-transparent'}`}><ShoppingBag size={24} className={activeTab === 'MARKET' ? 'fill-indigo-600 dark:fill-indigo-400' : ''} /></div>{activeTab === 'MARKET' && <span className="text-[10px] font-bold mt-1 animate-fade-in">Loja</span>}</button>
                    <button onClick={() => navigateTo('PROFILE')} className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300 active:scale-95 ${activeTab === 'PROFILE' ? 'text-indigo-600 -translate-y-2 dark:text-indigo-400' : 'text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}><div className={`p-2 rounded-xl transition-all ${activeTab === 'PROFILE' ? 'bg-indigo-100 dark:bg-indigo-900/60 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40' : 'bg-transparent'}`}><User size={24} className={activeTab === 'PROFILE' ? 'fill-indigo-600 dark:fill-indigo-400' : ''} /></div>{activeTab === 'PROFILE' && <span className="text-[10px] font-bold mt-1 animate-fade-in">Perfil</span>}</button>
                </div>
            </div>
